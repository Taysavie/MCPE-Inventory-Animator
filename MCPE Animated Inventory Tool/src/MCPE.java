...
public class MCPE {
	...
	public static void createUIDX(String pack) throws IOException {
		try {
			...
			// Only include the animated UI definitions we keep
	...
	public static void loadAsset(String path, String asset) {
		try {
			...
			// fallback: try copying from local workspace `assets` folder
			File srcFromAssets = new File("assets" + asset);
			if (srcFromAssets.exists()) {
				FileUtils.copyFile(srcFromAssets, dest);
				return;
			}
			// Try without leading slash
			File srcFromRoot = new File("." + asset);
			if (srcFromRoot.exists()) {
				FileUtils.copyFile(srcFromRoot, dest);
				return;
			}
			throw new IOException("Resource not found on classpath or disk: " + asset);
		} catch (Exception e) {
			JOptionPane.showMessageDialog(null, e.getMessage(), "Error!", JOptionPane.ERROR_MESSAGE);
			e.printStackTrace();
		}
